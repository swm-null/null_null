...
from ai.memo.utils.metadata_extractor.utils import image_to_text, link_to_text, voice_record_to_text
...
async def process_metadata(content: str, image_urls: list[str], voice_record_urls: list[str], lang: str="Korean") -> str:
    ...
    if voice_record_urls:
        tasks.append(asyncio.create_task(voice_record_to_text(voice_record_urls, lang)))
    if extracted_links:
