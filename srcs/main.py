...
@app.post("/search", response_model=Res_post_search, tags=["search"])
async def post_search(body: Arg_post_search):
    return await search_memo(body.content, body.user_id)

@app.post("/get-embedding", response_model=Res_get_embedding, tags=["embedding"])
def get_embedding(body: Arg_get_embedding):
    return Res_get_embedding(embedding=embedder.embed_query(body.content))

@app.post("/get-metadata-with-embedding", response_model=Res_get_metadata_with_embedding, tags=["embedding"])
...
@app.post("/memo/tags", response_model=Res_post_memo_tags, tags=["memo"])
...
@app.post("/memo/tag", response_model=Res_post_memo_tag, tags=["memo"])
async def post_memo_tag(body: Body_post_memo_tag):
    return Res_post_memo_tag(tags=await create_tag(body.user_id, body.raw_memo))

@app.post("/memo/structures", response_model=Res_post_memo_structures, tags=["memo"])
...
@app.post("/kakao-parser", response_model=Res_post_memo_structures, tags=["parser"])
