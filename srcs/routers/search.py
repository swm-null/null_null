...
async def post_search_ai(body: Arg_post_search_ai):
    return await search_memo_using_ai(body.query, body.user_id)

@router.post("/search/db", response_model=Res_post_search_db)
async def post_search_db(body: Arg_post_search_db):
    return await search_memo_using_db(body.query, body.user_id)
